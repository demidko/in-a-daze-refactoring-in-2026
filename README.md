...
<pre class="mermaid">
  sequenceDiagram
    ...
    participant Monolith as 🐘 Бекенд (PHP)
    ...
    Note right of User: Бекенд не умеет отображать PDF в диалоге
    ...
    Uploader-->>-Monolith: 5. Отдаёт идентификаторы бекенду
...
* Проблема добралась до программистов
...
* Бекенду таймаут в 15 секунд не подоходит
...
* Как наш repl работает?
  ...
  ```
* Этот подход ускоряет нам время на изолированных участках 
* По сути становясь функциональным тестом приложения (cl-like-exp)
...
* Но есть кое-что более важное в эпоху llm:
    ...
    * А в Rust применимо допущение, если скомпилировалось, то с очень большой вероятностью работает
...
![img_13.png](src/img_13y.png)

---

# Реакции ребят из php-команды 🙂
...
* Для бизнеса — ~~программистов~~ людей нужно будет меньше
...
* Порог входа в pet-проекты упал до нуля
    * Любой мейнстримный ЯП доступен каждому
    * ...даже если вы боялись конструкций вида `++*(&**ptr)`
* Когда разрабатываете что-то с llm, то автоматически, одновременно учитесь
* Необходимые в процессе знания таким образом напрямую попадают в кеш вашего мозга
* Llm становятся плагином-расширением мозга
* Стало возможно создавать более полезное, кастомное ПО, своими силами на выходных
  * Это в некотором роде, возвращает нас к тому с чего все когда-то начиналось

---

# Геометрическая прогрессия обучения

* Первые университеты
    * Расчитаны на дефицит книг
    * Один человек читает лекцию, остальные слушают и записывают для заучивания
* #### Развитие книгопечатания
    * Пропала объективная потребность в заучивании многих вещей
    * Однако человечество продолжало двигаться по инерции как раньше
* ### Интернет усиливает эффект книгопечатания в миллионы раз
    * Де-факто любые научные статьи стали бесплатны 24/7
    * Stackoverflow-driven-development уже в 2010ых стал мемом
* ## Появление llm — закономерная вишенка на торте
    * Школы, вузы и курсы стали очевидной профанацией
    * Полная десакрализация передачи знания, обесценивание индустрии образования
    * Потрясающие возможности учиться, создавать что-то свое, всегда, для всех и каждого

---

# Итак, будет надеяться llm устранит...

* Копирайтеров графоманов
* Черных SEO, загадивших выдачу google
* Психологов, коучей и продавцов курсов успешного успеха
* И всех прочих инфоцыган, которых вспоминать в приличном обществе не принято
